...
const ChatInterface = () => {
  ...
  const handleDeleteMessage = async (messageId: string) => {
    ...
    const { error } = await supabase.rpc('delete_message_admin', {
        ...
        <div className="bg-surface border border-metallic/30 rounded-lg p-8 max-w-md w-full mx-4">
          ...
          <div className="space-y-4">
            <Input
              ...
              className="bg-surface-elevated border-metallic/30 text-text-primary placeholder:text-text-muted font-mono text-right"
              dir="rtl"
              maxLength={32}
            />
            <div className="text-xs text-text-muted font-mono" dir="rtl">
              استخدم "مدير" كاسم رمزي للوصول الإداري
            </div>
            <Button
        ...
        <div className="p-4 bg-surface-elevated border-t border-metallic/20">
          <div className="flex gap-3">
            <Input
              ...
              className="flex-1 bg-surface border-metallic/30 text-text-primary placeholder:text-text-muted font-mono text-right"
              dir="rtl"
            />
            <Button
          ...
          <div className="text-xs text-text-muted font-mono mt-2" dir="rtl">
            الرسائل مشفرة • محادثة صوتية متاحة • مراقبة إدارية نشطة
          </div>
        </div>
      </>
    );
